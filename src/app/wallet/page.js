...
export default async function WalletPage() {
  const { wallets, transactions } = await getWallet();
  ...
  return (
    <div className="animate-fadeup">
      ...
      <div className="mb-6 grid grid-cols-2 gap-4">
        {Object.entries(wallets).map(([addr, w]) => (
          <div
            key={addr}
          ...
          >
            ...
            <div className="mt-1 font-mono text-[0.6rem] text-[#7a7570]">{addr}</div>
          </div>
        ))}
      </div>
      ...
      <Link href="/payment" className="inline-block rounded-lg bg-[#C9A84C] px-8 py-3.5 text-xs font-medium tracking-widest uppercase text-black transition-all hover:bg-[#E8C97A] hover:-translate-y-0.5">
        Buat Transaksi MED →
      </Link>
    </div>
  );
